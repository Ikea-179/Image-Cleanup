function output = conv(img, mask)
    ...
    % Add 0 padding for the image to allow for 
    % convolutions at the edges
    expanded = add_paddings(img, row_offset, col_offset);
    % initialize the output image
    output = zeros(num_img_rows, num_img_cols, 'uint8');

    % Iterate over all pixels
    for y=1+col_offset:num_img_rows+1
        for x=1+row_offset:num_img_cols+1
            y0 = y-row_offset;
            x0 = x-col_offset;
